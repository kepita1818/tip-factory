...
async function loadMatches(leagueId, leagueName) {
    ...
    matchesTitle.textContent = leagueName;
    showMatches();
    ...
    try {
        ...
        console.error(e);
        matchesList.innerHTML = '<div class="no-matches">No hay partidos disponibles</div>';
    }
}
...
function renderMatches(matches) {
    ...
    matchesList.innerHTML = matches.map(match => {
        ...
        const date = match.utcDate ? match.utcDate.substring(0, 10) : '';
        
        return `
            <div class="match-card" onclick="analyzeMatch(${match.id})">
                <div class="match-card-teams">
                    <div class="match-card-team">
                        <img src="${home.crest || ''}" alt="${home.name}" onerror="this.src='https://via.placeholder.com/32?text=?'">
                        <span>${home.shortName || home.name}</span>
                    </div>
                    <div class="match-card-vs">VS</div>
                    <div class="match-card-team away">
                        <img src="${away.crest || ''}" alt="${away.name}" onerror="this.src='https://via.placeholder.com/32?text=?'">
...
async function analyzeMatch(matchId) {
    ...
    // Show loading state
    document.getElementById('home-name').textContent = 'Cargando...';
    document.getElementById('away-name').textContent = 'Cargando...';
...
function renderAnalysis(data) {
    ...
    document.getElementById('home-logo').src = info.home_logo || 'https://via.placeholder.com/60?text=?';
    document.getElementById('away-logo').src = info.away_logo || 'https://via.placeholder.com/60?text=?';
    document.getElementById('match-meta').textContent = `${info.date} | ${info.league} | ${info.time}`;
    
    // Table headers
    const homeShort = info.home_team.substring(0, 12);
    const awayShort = info.away_team.substring(0, 12);
